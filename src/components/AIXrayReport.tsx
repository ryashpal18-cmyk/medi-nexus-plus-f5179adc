...
Examine ALL visible structures. Respond ONLY with valid JSON (no markdown, no extra text):
    ...
    <div style={{ background: "#f8fafc", border: "1px solid #dbeafe", borderRadius: 16, padding: 16, marginBottom: 24 }}>
      {showPayment && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,.62)", zIndex: 9999, display: "grid", placeItems: "center", padding: 16 }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: 20, width: "min(420px,100%)", boxShadow: "0 24px 80px rgba(15,23,42,.35)" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, background: "#eff6ff", padding: 10, borderRadius: 10, marginBottom: 12 }}><div><span style={{ display: "block", fontSize: 11, color: "#64748b" }}>UPI ID</span><b>{UPI_ID}</b><div style={{ color: "#0369a1", fontWeight: 700 }}>Amount: ₹{REPORT_FEE}</div></div><button onClick={() => navigator.clipboard.writeText(UPI_ID)} style={{ background: "#3b82f6", color: "#fff", border: "none", borderRadius: 6, padding: "6px 12px", cursor: "pointer", fontSize: 12, fontWeight: 600 }}>Copy</button></div>
            ...
            <button onClick={handlePaymentConfirm} disabled={verifying} style={{ width: "100%", padding: 12, border: "none", borderRadius: 10, color: "#fff", background: "linear-gradient(90deg,#16a34a,#22c55e)", fontWeight: 800, cursor: "pointer" }}>{verifying ? "⏳ Verify ho rahi hai..." : "✅ Maine ₹50 Pay Kar Di - Generate Karo"}</button>
      ...
      <div style={{ background: "#fff", borderRadius: 12, padding: 16, marginBottom: 16, border: "1px solid #e2e8f0" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 12, marginBottom: 12 }}>
          ...
          <label><span style={lbl}>Gender</span><select style={inp} value={gender} onChange={(e) => setGender(e.target.value)}><option>Male</option><option>Female</option><option>Other</option></select></label>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12, marginBottom: 12 }}>
          <label><span style={lbl}>Body Part</span><select style={inp} value={body} onChange={(e) => setBody(e.target.value)}>{BODY_PARTS.map((b) => <option key={b}>{b}</option>)}</select></label>
          <label><span style={lbl}>View</span><select style={inp} value={view} onChange={(e) => setView(e.target.value)}>{VIEWS.map((v) => <option key={v}>{v}</option>)}</select></label>
        </div>
      ...
      {report && <div style={{ background: "#fff", border: "1px solid #bfdbfe", borderRadius: 14, overflow: "hidden" }}>
        <div style={{ background: "linear-gradient(90deg,#1e3a8a,#0369a1)", color: "#fff", padding: 16, display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
          <div><div style={{ fontSize: 18, fontWeight: 800 }}>📋 {CLINIC_NAME}</div><div style={{ fontSize: 12, opacity: 0.9 }}>{reportId.current} · {dateStr} · {timeStr}</div></div>
